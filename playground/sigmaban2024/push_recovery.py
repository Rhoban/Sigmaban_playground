...
# ==============================================================================
"""PushRecovery task for Sigmaban. (based on Berkeley Humanoid)"""
...
def default_config() -> config_dict.ConfigDict:
    return config_dict.create(
        ...
        push_config=config_dict.create(
            ...
            magnitude_range=[0.3, 1.5],  # was [0.3, 1.0]
        ),
        # UNUSED
        lin_vel_x=[-0.2, 0.3],
        lin_vel_y=[-0.2, 0.2],
...
class PushRecovery(sigmaban_base.SigmabanEnv):
    """Recover from a front or back push"""
    ...
    def _post_init(self) -> None:
        ...
        if USE_IMITATION_REWARD:
            ...
            self.dx_range = jp.array([0.0, 0.0])
            self.dy_range = jp.array([0.0, 0.0])
            self.dtheta_range = jp.array([0.0, 0.0])
    ...
    def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
        ...
        # push theta is either 0 or pi, chosen randomly
        push_theta = jp.where(
            jax.random.uniform(push1_rng, shape=()) < 0.5,
            0.0,
            jp.pi,
        )
